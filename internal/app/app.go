...
import (
	...
	"github.com/lacsar712/asphaltm/internal/aggregate"
	"github.com/lacsar712/asphaltm/internal/mixdrum"
	"github.com/lacsar712/asphaltm/internal/fsm"
	"github.com/lacsar712/asphaltm/internal/interlock"
...
type App struct {
	...
	plant        *mixdrum.MixPlant
	towerFSM     *fsm.TowerFSM
	zones        *mixdrum.ZoneTable
	...
	holdMgr      *bindertemp.HoldWindowManager
	holdEval     *bindertemp.HoldWindowEvaluator
	gradAudit    *bindertemp.OrderedGradientValidator
...
func New(cfg config.Config) (*App, error) {
	...
	zones, err := mixdrum.NewZoneTable(towerID, cfg.ZoneCount, plenumID, cfg)
	if err != nil {
		return nil, err
	}
	plant := mixdrum.NewMixPlant(cfg, clk, mem)
	...
	for i, z := range zones.Zones() {
		...
		plant.RegisterSensor(bindertemp.NewSensor(sensorID, z.Zone))
		targets = append(targets, cfg.TargetMoistPct)
	}
	if err := plant.InitProfile(zoneIDs, targets); err != nil {
		return nil, err
	}
	ventBank, err := aggregate.NewVentBank(zoneIDs, "vent-damper")
	if err != nil {
		return nil, err
	}
	...
	holdMgr := bindertemp.NewHoldWindowManager()
	a := &App{
		...
		gradAudit:    bindertemp.NewOrderedGradientValidator(zoneIDs, cfg.MaxGradientDeltaPct, 0.5),
		guard:        interlock.NewGuard(guardPairs),
		batchCancels: make(map[model.TowerID]context.CancelFunc),
	}
	a.aggregate = initDehumidController(ventBank, cfg.MaxGradientDeltaPct, func(readings []model.MoistureReading) float64 {
	...
	a.dryFSM = fsm.NewPlantFSM(towerID, a.onDryTransition)
	fsm.RegisterDryHeatHook(a.dryFSM.Hooks())
	a.dryRamp = NewMixRamp(a.clk, cfg.ProcessTick(), cfg.MixRampSteps)
