...
func (a *App) observeZoneMoisture(ctx context.Context) error {
	...
	if err := a.aggregate.ApplyCycle(ctx, readings); err != nil {
		return err
	}
	return a.aggregate.ValidateGradient(readings)
}
...
func (a *App) routeSummaryLine(plenum model.PlenumID) string {
	...
	return fmt.Sprintf("routes=%d active_vents=%d fan_load=%.0f",
		summary.RouteCount, a.aggregate.Bank().ActiveCount(), a.stager.CurrentLoad())
}
...
func initHoldEvaluator(profile *bindertemp.ProfileManager, windows *bindertemp.HoldWindowManager) *bindertemp.HoldWindowEvaluator {
	return bindertemp.NewHoldWindowEvaluator(profile, windows)
}

func initDehumidController(bank *aggregate.VentBank, maxDelta float64, gradientFn func([]model.MoistureReading) float64) *aggregate.Controller {
	c := aggregate.NewController(bank, maxDelta)
